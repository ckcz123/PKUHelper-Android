	...
	protected void onPostExecute(Parameters parameters) {
		...
		if (!"200".equals(parameters.name)) {
			if ("-1".equals(parameters.name)) {
				CustomToast.showInfoToast(LostFoundActivity.lostFoundActivity, "无法连接网络(-1,-1)");
			}
			else
				CustomToast.showInfoToast(LostFoundActivity.lostFoundActivity, "无法连接到服务器 (HTTP "+parameters.name+")");
		}
		else LostFoundActivity.lostFoundActivity.finishRequest(requestType,parameters.value);
	}
