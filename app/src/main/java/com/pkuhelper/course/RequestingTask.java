...
public class RequestingTask extends AsyncTask<ArrayList<Parameters>, String, Parameters>{
	...
	public RequestingTask(String msg,String url,int type) {
		progressDialog=new ProgressDialog(CustomCourseActivity.customCourseActivity);
	...
	protected void onPostExecute(Parameters parameters) {
		...
		if (!"200".equals(parameters.name)) {
			if ("-1".equals(parameters.name))
				CustomToast.showInfoToast(CustomCourseActivity.customCourseActivity, "无法连接网络(-1,-1)");
			else 
				CustomToast.showInfoToast(CustomCourseActivity.customCourseActivity, "无法连接到服务器 (HTTP "+parameters.name+")");
		}
		else CustomCourseActivity.customCourseActivity.finishRequest(requestType,parameters.value);
	}
