	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		if (id==Constants.MENU_COURSE_ADD) {
			...
			.setItems(strings, new DialogInterface.OnClickListener() {
				...
				@Override
				public void onClick(DialogInterface dialog, int which) {
					if (which==0) startActivity(new Intent(pkuhelper, DeanCourseActivity.class));
					else if (which==1) startActivity(new Intent(pkuhelper, CustomCourseActivity.class));
					else if (which==2) startActivity(new Intent(pkuhelper, ExamActivity.class));
				}
			}).show();
